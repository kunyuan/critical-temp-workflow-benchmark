...
The compact U(1) gauge theory in four Euclidean dimensions is simulated using the Villain-equivalent world-sheet formulation, where integer-valued plaquette multiplicities n_p subject to the closed-surface constraint are updated by Metropolis on L^4 hypercubic lattices with periodic boundary conditions, for L from 6 to 24. The specific-heat maximum, Lee-Yang zeroes, energy histograms (latent heat), and autocorrelation times are analyzed. What is the ORDER of the confinement-to-Coulomb (deconfinement) phase transition of the compact U(1) gauge theory in this Villain formulation? Answer in JSON with field "transition_order" using one of: "first_order", "continuous", "bkt".
