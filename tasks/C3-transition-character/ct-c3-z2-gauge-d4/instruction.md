...
The pure Z_2 lattice gauge theory in four space-time dimensions (d=4) is simulated by Metropolis Monte Carlo with thermal cycles and quench experiments on 4^4 hypercubic lattices. The model has gauge coupling K and no matter (Higgs coupling h=0). The transition separates an electric confinement regime (small K) from an unconfined regime (large K). What is the ORDER of the bulk phase transition in the pure d=4 Z_2 lattice gauge theory? Answer in JSON with field "transition_order" using one of: "first_order", "continuous", "bkt".
