...
Consider the ferromagnetic Ising model on a four-dimensional hypercubic lattice. The Hamiltonian is H = -J * sum_{<i,j>} sigma_i sigma_j, where sigma_i = +/-1 and the sum runs over all nearest-neighbor pairs on the 4D hypercubic lattice with coupling J > 0. What is the critical temperature T_c in units of J/k_B (i.e., k_B T_c / J), determined via the higher-order tensor renormalization group (HOTRG) at bond dimension D_cut=13 and effective system size N = 1024^4 lattice sites? Output a JSON object {"kBTc_over_J": <value>}.
