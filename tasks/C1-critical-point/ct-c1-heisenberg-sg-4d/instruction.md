...
Consider the short-range isotropic Heisenberg spin glass on a four-dimensional hypercubic lattice. The Hamiltonian is H = -(1/2) * sum_{<i,j>} J_{ij} * sigma_i . sigma_j, where each spin sigma_i is a three-component unit vector (sigma_i in S^2), the sum runs over nearest-neighbor pairs on the 4D hypercubic lattice (V = L^4) with periodic boundary conditions, and the exchange couplings J_{ij} are independent Gaussian random variables with zero mean and unit variance: P(J_{ij}) = (1/sqrt(2pi)) * exp(-J_{ij}^2 / 2). Simulated Tempering Monte Carlo simulations are performed on lattice sizes L = 3, 4, 5. Report k_B*T_c/J (with k_B=1 and J the coupling scale = the standard deviation of the Gaussian couplings = 1) for the spin-glass critical temperature determined from finite-size scaling of the Binder cumulant g(L,T) = (1/2)*(11 - 9*<q^4>_bar / (<q^2>_bar)^2), where q is the scalar overlap? Output a JSON object {"kBTc_over_J": <value>}.
