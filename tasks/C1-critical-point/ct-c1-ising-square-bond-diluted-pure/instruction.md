...
Consider the ferromagnetic Ising model on a two-dimensional square lattice. Each site carries a spin S_i = +/-1. The Hamiltonian is H = -J * sum_{<i,j>} S_i S_j where each nearest-neighbor bond is present with probability sigma and absent with probability (1-sigma). For the pure (undiluted) case sigma = 1.0, free boundary conditions are used, and lattice sizes L = 256, 1024, 4096 are studied. What is the critical temperature T_c in units of J/k_B (i.e., k_B T_c / J), determined via Binder cumulant crossing? Output a JSON object {"kBTc_over_J": <value>}.
