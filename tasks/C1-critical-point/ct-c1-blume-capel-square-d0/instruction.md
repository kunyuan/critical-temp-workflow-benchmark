...
Consider the spin-1 Blume-Capel model on a two-dimensional square lattice. The Hamiltonian is H = -J * sum_{<i,j>} S_i S_j + D * sum_i (S_i)^2, where S_i in {-1, 0, +1}, J is the exchange interaction, D is the crystal field, and the sum <i,j> runs over nearest-neighbor pairs on the square lattice. Periodic boundary conditions are used with linear sizes L = 8, 16, 32, 64, 128. At crystal field D = 0 and zero longitudinal magnetic field h = 0, what is the critical temperature k_B T_c / J, determined via the crossing point of the fourth-order Binder cumulant V_L = 1 - <M^4> / (3 <M^2>^2) for different L? Output a JSON object {"kBTc_over_J": <value>}.
