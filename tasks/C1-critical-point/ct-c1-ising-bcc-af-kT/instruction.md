...
Consider the nearest-neighbor antiferromagnetic Ising model on a body-centered cubic (BCC) lattice. Each site carries a spin S_i = +/-1, and nearest-neighbor pairs interact via H = -K * sum_{<i,j>} S_i S_j with exchange coupling constant K > 0 (ferromagnetic sign convention, so the AF ordering uses -K with K > 0 defining the energy scale). The lattice has periodic boundary conditions and linear sizes N up to 16. What is the infinite-lattice critical temperature T_inf estimated from finite-size scaling of the specific-heat peak positions, expressed as k*T_inf / K? Output a JSON object {"kTc_over_K": <value>}.
