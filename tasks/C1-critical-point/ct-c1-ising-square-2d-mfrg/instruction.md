...
Consider the ferromagnetic Ising model on a two-dimensional square lattice. Spins S_i = +/-1; Hamiltonian H = -J sum_{<ij>} S_i S_j with J>0 and periodic boundary conditions. The reduced coupling K = beta*J = J/(k_B*T). Using the three-cluster molecular-field renormalization group (MFRG) method with clusters of sizes L, L'=L-1, L''=L-2 for L=3 to 11: (1) compute the two-point correlation Theta(K) for each cluster size using the transfer matrix, (2) solve the MFRG fixed-point equations for K*, (3) extrapolate K*(L) to L→∞ using the alternating epsilon-algorithm. Determine the extrapolated critical reduced coupling K_c = J/(k_B T_c) (quantity: Kc). Output a JSON object {"Kc": <value>}.
