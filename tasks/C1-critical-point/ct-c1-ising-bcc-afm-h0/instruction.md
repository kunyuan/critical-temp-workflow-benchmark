...
Consider the three-dimensional antiferromagnetic Ising model on a body-centered cubic (BCC) lattice. The Hamiltonian is H = J1*sum_{<i,j>1} S_i S_j + J2*sum_{<i,j>2} S_i S_j, where S_i = +/-1, J1 > 0 and J2 > 0 are antiferromagnetic (positive) nearest-neighbor and next-nearest-neighbor exchange couplings with ratio r = J2/J1 = 1.0, the sums run over first and second nearest-neighbor pairs on the BCC lattice, and periodic boundary conditions are used. At zero external magnetic field (H=0), what is the critical (Neel) temperature T_N in units of k_B T_N / |J1|? Output a JSON object {"kBTN_over_J1": <value>}.
